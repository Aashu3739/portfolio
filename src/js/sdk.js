...
function compose_conference_li(data) {
  let content = '<li class="li">';
  content += `<p class="li_header"><b>${data.title}</b></p>`;
  content += `<p class="li_subtext">${data.conference}</p>`;
  if (data.link) {
    content += `<p><a href="${data.link}" target="_blank" rel="noopener noreferrer">View Publication</a></p>`;
  }
  content += '</li>';
  return content;
}
